...
func TestShouldSetMemberNickname(t *testing.T) {
	if (&Config{}).shouldSetMemberNickname() {
		t.Error("an empty template should leave member events alone")
	}
	if !(&Config{MemberNicknameTemplate: `{{.FullName}}`}).shouldSetMemberNickname() {
...
// The nickname must not leak into the shared ghost profile, which is the whole point.
