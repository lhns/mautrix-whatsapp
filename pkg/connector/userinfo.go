...
// updateDMPortalNames applies dm_room_name_template to a contact's existing DM rooms. The caller
// updates the ghost first and bridgev2 renames DM rooms from the ghost as soon as its name
// changes, so this has to run in the same pass rather than being queued.
func (wa *WhatsAppClient) updateDMPortalNames(ctx context.Context, jid types.JID) {
	...
	for _, userID := range userIDs {
		...
		// The source stays nil: UpdateInfo runs MarkInPortal for a non-nil source, which joins
		// that login's double puppet to the room.
