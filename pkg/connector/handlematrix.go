...
func (wa *WhatsAppClient) HandleMatrixRoomName(ctx context.Context, msg *bridgev2.MatrixRoomName) (ok bool, retErr error) {
	...
	// The Matrix name is rendered from group_room_name_template, so sending it back would store
	// the rendered name as the WhatsApp group subject and then render that again.
	if wa.Main.Config.shouldSetGroupRoomName() {
