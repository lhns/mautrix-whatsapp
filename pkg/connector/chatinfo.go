...
func (wa *WhatsAppClient) wrapDMInfo(ctx context.Context, jid types.JID) *bridgev2.ChatInfo {
	...
	if wa.Main.Config.shouldSetDMRoomName(wa.Main.Bridge.Config.PrivateChatPortalMeta) && !wa.IsOwnJID(jid) {
...
// memberNickname renders member_nickname_template for a contact, or "" if the contact cannot be
// read. Empty leaves ChatMember.Nickname nil, so bridgev2 leaves the member event alone.
...
func (wa *WhatsAppClient) wrapGroupInfo(ctx context.Context, info *types.GroupInfo) *bridgev2.ChatInfo {
	...
	var hasSelf bool
	setNicknames := wa.Main.Config.shouldSetMemberNickname()
